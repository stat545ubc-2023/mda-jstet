...
As I am intereseted in tree diversity, it makes sense to calculate how many trees if each species occur in each neighborhood.
```{r include=TRUE}
...
Computing summariy statistics for diameter makes sense for this task because it allows us to compare this across neighborhoods. Diamater is a first indicator for tree growth speed.
```{r include=TRUE}
...
Visualizing for one tree only for now so that growth speed differences between species do not bias the result.
```{r include=TRUE}
...
**Research Question**: Does the existence of a root barrier have an influence on diameter? 
